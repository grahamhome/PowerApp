...
get_csvdata_pmuattack <- function(){
  ...
  Freq <<- read.csv("data/rawdata/BPA_230kV_data/weccbpaabove230_open_gen_pmu_freq.csv")
  Pangle <<- read.csv("data/rawdata/BPA_230kV_data/weccbpaabove230_open_gen_pmu_vang_error.csv")
  Volt <<- read.csv("data/rawdata/BPA_230kV_data/weccbpaabove230_open_gen_pmu_vmag_error.csv")
}


#Change up the names so that they all match
clean_names_pmuattack<- function(){
...
get_merged_data_pmuattack<- function(){
...
get_map_data_pmuattack  <- function(){
...
get_rocdata_pmuattack <- function(){
  Dv <<- as.matrix(Volt[,-1])
  Cv <<- Volt[1,]
  currt_voltdiff <<- 1
...
import_data <- function(){
  get_csvdata_pmuattack()
  clean_names_pmuattack()
  get_merged_data_pmuattack()
  get_map_data_pmuattack()
  get_rocdata_pmuattack()
}
