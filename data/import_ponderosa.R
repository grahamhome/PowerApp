...
get_csvdata_ponderosa <- function(){
  ...
  Freq <<- read.csv("data/rawdata/BPA_230kV_data/weccbpaabove230_ponderosa_pmu_freq.csv")
  Pangle <<- read.csv("data/rawdata/BPA_230kV_data/weccbpaabove230_ponderosa_pmu_vang.csv")
  Volt <<- read.csv("data/rawdata/BPA_230kV_data/weccbpaabove230_ponderosa_pmu_vmag.csv")
}


#Change up the names so that they all match
clean_names_ponderosa  <- function(){
...
get_merged_data_ponderosa <- function(){
...
get_map_data_ponderosa  <- function(){
...
get_rocdata_ponderosa <- function(){
  Dv <<- as.matrix(Volt[,-1])
  Cv <<- Volt[1,]
  currt_voltdiff <<- 1
...
import_data <- function(){
  get_csvdata_ponderosa()
  clean_names_ponderosa()
  get_merged_data_ponderosa()
  get_map_data_ponderosa()
  get_rocdata_ponderosa()
}
